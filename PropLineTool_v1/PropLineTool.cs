...
namespace PropLineTool
{
    ...
    public class PropLineTool : ToolBase
    {
        ...
        public static float hoverCurveT = 0f;
        ...
        public struct ItemPlacementInfo
        {
            ...
            public void SetTAndPosition(float t)
            {
                ...
                else if (drawMode == DrawMode.Curved || drawMode == DrawMode.Freeform)
                {
        ...
        public class PlacementCalculator
        {
            ...
            private float m_lastFinalOffset;
            private float lastFinalOffset
            {
                get
                {
                    return m_lastFinalOffset;
                }
                set
                {
                    float _oldValue = m_lastFinalOffset;
            ...
            //used in fence mode
            private Vector3 m_lastFenceEndpoint;
            private Vector3 lastFenceEndpoint
            {
                get
                {
                    return m_lastFenceEndpoint;
                }
                set
                {
                    Vector3 _oldValue = m_lastFenceEndpoint;
            ...
            private float m_newFinalOffset;    //used in non-fence mode
            private Vector3 m_newFenceEndpoint;    //used in fence mode

            public event VoidEventHandler eventLastContinueParameterChanged;
            ...
            public Randomizer randomizerFresh
            {
                get
                {
                    switch (objectMode)
            ...
            private bool m_isContinueDrawing;
            private bool m_keepLastOffsets = false;

            //continue drawing getters
            public bool isContinueDrawing
            {
                get
                {
                    return m_isContinueDrawing;
                }
            }
            public bool keepLastOffsets
            {
                get
                {
                    return m_keepLastOffsets;
                }
            }

            //error checking bools
            private bool m_allItemsValid = true;
            /// <summary>
            ...
            public bool allItemsValid
            {
                get
                {
                    return m_allItemsValid;
                }
                private set
                {
                    m_allItemsValid = value;
                }
            }
            ...
            internal float spacingSingle
            {
                ...
                set
                {
                    ...
                    value = Mathf.Clamp(value, 0.10f, 2000f);
            ...
            protected void OnBaseParameterChanged()
            {
                ...
                UpdateItemPlacementInfo(m_isContinueDrawing, m_keepLastOffsets);
            }
            ...
            {
                m_isContinueDrawing = continueDrawing;
            }

            internal Vector3 GetLastFenceEndpoint()
            {
                return lastFenceEndpoint;
            }
            ...
            {
                return UpdateItemPlacementInfo(m_isContinueDrawing, m_keepLastOffsets);
            }

            public bool UpdateItemPlacementInfo(bool forceContinueDrawing)
            {
                return UpdateItemPlacementInfo(forceContinueDrawing, m_keepLastOffsets);
            }
            ...
            public bool UpdateItemPlacementInfo(bool forceContinueDrawing, bool forceKeepLastOffsets)
            {
                ...
                {
                    m_keepLastOffsets = true;
                    result = CalculateAll(true);
                }
                else
                {
                    m_keepLastOffsets = forceKeepLastOffsets;
                    result = CalculateAll(forceContinueDrawing || m_isContinueDrawing);
                }

                m_isContinueDrawing = forceContinueDrawing;
            ...
            /// <returns></returns>
            private bool CalculateAllPositions(float initialOffset, Vector3 lastFenceEndpoint)
            {
                int _numItems = 0;
                float _finalT = 0f;
                float _deltaT = 0f;
                ...
                if (drawMode == DrawMode.Straight)
                {
                    if (fenceMode == true)
                    {
                        if (GetCurveLength(m_mainSegment) < 0.75f * m_spacingSingle)
                        {
                            m_itemCount = 0;
                            return false;
                        }
                    ...
                    {
                        if (GetCurveLength(m_mainSegment) < m_spacingSingle)
                        {
                            m_itemCount = 0;
                            return false;
                        }
                ...
                {
                    if (GetCurveLength(m_mainBezier) < m_spacingSingle)
                    {
                        m_itemCount = 0;
                        return false;
                    }
                    ...
                    {
                        if(Vector3.Distance(m_mainBezier.d, m_mainBezier.a) < m_spacingSingle)
                        {
                            m_itemCount = 0;
                            return false;
                        }
                ...
                if (fenceMode == true)   //FenceMode = ON
                {
                    ...
                    if (drawMode == DrawMode.Straight)
                    {
                        ...
                        float _numItemsFloat = _lengthFull / m_spacingSingle;
                        _numItems = Mathf.Min(m_itemCount, Mathf.Clamp(Mathf.FloorToInt(_numItemsFloat), 0, MAX_ITEM_ARRAY_LENGTH));

                        //add an extra item at the end if within 75% of spacing
                        float _remainder = _lengthFull % m_spacingSingle;
                        float _remFraction = _remainder / m_spacingSingle;
                        if (_remFraction >= 0.75f)
                        {
                            _numItems += 1;
                        }
                        ...
                        //distance = speed * t
                        //t = distance / speed
                        _deltaT = m_spacingSingle / _speed;
                    ...
                    else if (drawMode == DrawMode.Curved || drawMode == DrawMode.Freeform)
                    {
                        ...
                        _numItems = Mathf.Min(m_itemCount, Mathf.Clamp(Mathf.CeilToInt(_lengthFull / m_spacingSingle), 0, MAX_ITEM_ARRAY_LENGTH));
                        ...
                        if (initialOffset > 0f && lastFenceEndpoint != Vector3.down)
                        {
                            ...
                            if (!Math.MathPLT.LinkCircleCurveFenceIntersectXZ(m_mainBezier, lastFenceEndpoint, m_spacingSingle, m_tolerance, out _t, false))
                            {
                            ...
                            if (!Math.MathPLT.CircleCurveFenceIntersectXZ(m_mainBezier, _t, m_spacingSingle, m_tolerance, out _t, false))
                            {
                                //failed to converge
                                _numItems = 1;
                                goto label_endpointsFinish;
                    ...
                    label_endpointsForLoop:
                        ...
                        //then calculate midpoints
                        Vector3 _midpoint = this.lastFenceEndpoint;
                        for (int i = 0; i < _numItems; i++)
                        {
                            _midpoint = Vector3.Lerp(GetFenceEndpoint(i), GetFenceEndpoint(i + 1), 0.50f);
                    ...
                    if (_numItems > 0 && _numItems <= MAX_ITEM_ARRAY_LENGTH)
                    {
                        ...
                        m_newFenceEndpoint = _finalEndpointPos;

                        Vector3 _finalPos = GetItemPosition(_numItems - 1);
                        m_newFinalOffset = Vector3.Distance(_finalPos, m_mainSegment.b);
                    }
                    else
                    {
                        m_itemCount = 0;
                ...
                else   //Non-fence mode
                {
                    ...
                    if (drawMode == DrawMode.Straight)
                    {
                        ...
                        _numItems = Mathf.Min(m_itemCount, Mathf.CeilToInt(_lengthAfterFirst / m_spacingSingle));
                        ...
                        if (initialOffset > 0f)
                        {
                            ...
                            float _initialT = initialOffset / _speed;
                            _t = _initialT;
                        }
                        ...
                        {
                            m_newFinalOffset = m_spacingSingle - Vector3.Distance(_finalPos, m_mainSegment.b);
                        }
                    }
                    // ====== CURVED/FREEFORM ======
                    else if (drawMode == DrawMode.Curved || drawMode == DrawMode.Freeform)
                    {
                        ...
                        for (int i = 0; i < _numItems; i++)
                        {
                            _position = m_mainBezier.Position(_t);
                            SetTAndPosition(i, _t, _position);
                            Math.MathPLT.StepDistanceCurve(m_mainBezier, _t, m_spacingSingle, m_tolerance, out _t);
                        }
                        ...
                        else
                        {
                            ...
                            m_newFinalOffset = m_spacingSingle - Math.MathPLT.CubicBezierArcLengthXZGauss04(m_mainBezier, _finalT, 1f);
                        }
                    }
        ...
        private bool IsCurveLengthLongEnoughXZ()
        {
            bool _result = false;
        ...
        protected override void Awake()
        {
            ...
            {
                if (mode == DrawMode.Straight && fenceMode == true && (activeState == ActiveState.CreatePointSecond || activeState == ActiveState.CreatePointThird))
                {
                    ...
                    case 0:
                        {
                            ...
                            bool _isSnapFirstControlPoint = ((_pointCount == 1 && activeState == ActiveState.CreatePointSecond) || (_pointCount == 2 && activeState == ActiveState.CreatePointThird)) && (drawMode == DrawMode.Straight);
                ...
                case DrawMode.Straight:
                    {
                        if (_fenceMode)
                        {
                            _p1.m_position = placementCalculator.GetLastFenceEndpoint();
                        }
                ...
                case ActiveState.LockIdle:
                    {
                        ...
                        switch (drawMode)
                        {
                            case DrawMode.Straight:
                                {
                                    //PropLineToolMod.optionPanel.LockModeButtonEnabler(DrawMode.Curved, false);
                                    //PropLineToolMod.optionPanel.LockModeButtonEnabler(DrawMode.Freeform, false);
                                    break;
                                }
                            case DrawMode.Curved:
                                {
                                    //PropLineToolMod.optionPanel.LockModeButtonEnabler(DrawMode.Straight, false);
                                    //PropLineToolMod.optionPanel.LockModeButtonEnabler(DrawMode.Freeform, false);
                                    break;
                                }
                            case DrawMode.Freeform:
                        ...
                        if (placementCalculator.GetItemCountActual() >= (1 + _offset))
                        {
                            ...
                            hoverAngle = m_placementInfo[1].angle - placementCalculator.totalPropertyAngleOffset + Mathf.PI;
                        }
                        break; 
                    }
                case ActiveState.MovePointFirst:
                ...
                case ActiveState.ChangeAngle:
                    {
                        ...
                        lockBackupItemSecondAngle = m_placementInfo[1].angle;
                        lockBackupItemDirection = m_placementInfo[1].itemDirection;
                        break; 
                    }
                default:
                    {
                ...
                case ActiveState.CreatePointSecond:
                    {
                        if (!_isInsideUI)
                        {
                            if (key._ctrlOnlyLeftClick && drawMode == DrawMode.Straight)
                            {
                            ...
                            {
                                if (PropLineTool.drawMode == PropLineTool.DrawMode.Straight)
                                {
                                    //check if curve is too short
                                    if (IsCurveLengthLongEnoughXZ() == false)
                                    {
                                    ...
                                    if (this.m_mouseRayValid && _addCPResult)
                                    {
                                        ...
                                        if (PropLineTool.m_lockingMode == LockingMode.Off)
                                        {
                                            ...
                                            if (_continueCPResult)
                                            {
                                                ...
                                                if (fenceMode == true && placementCalculator.isContinueDrawing == true)
                                                {
                                    ...
                                    if (PropLineTool.m_lockingMode == LockingMode.Off)
                                    {
                                        ...
                                        if (_continueCPResult)
                                        {
                                            ...
                                            else if (PropLineTool.drawMode == DrawMode.Freeform)
                                            {
                                                ...
                                                Vector3 _vector2 = _temp2;
                                                ModifyControlPoint(_vector2, 3);
                ...
                case ActiveState.LockIdle:
                    {
                        if (!_isInsideUI)
                        {
                            if (key._ctrlEnter || key._ctrlOnlyLeftClick)
                            {
                                ...
                                if (PropLineTool.drawMode == DrawMode.Straight)
                                {
                                    if (_isMouseRayValid)
                                    {
                                        _continueCPResult = ContinueDrawing();
        ...
        protected override void OnToolLateUpdate()
        {
            ...
            //check if user switched from Curved/Freeform(in CreatePointThird) -> Straight
            if (drawMode == DrawMode.Straight && activeState == ActiveState.CreatePointThird)
            {
            ...
            //check if user switched from Straight(in CreatePointSecond and continueDrawing fenceMode) -> Curved/Freeform
            if ((drawMode == DrawMode.Curved || drawMode == DrawMode.Freeform) && activeState == ActiveState.CreatePointSecond && placementCalculator.IsPositionEqualToLastFenceEndpoint(m_cachedControlPoints[0].m_position))
            {
                placementCalculator.ResetLastContinueParameters();
            }
            if (activeState == ActiveState.CreatePointFirst && placementCalculator.IsPositionEqualToLastFenceEndpoint(m_cachedControlPoints[0].m_position))
            {
                placementCalculator.ResetLastContinueParameters();
            }
        ...
        public void FinalizePlacement(bool continueDrawing, bool isCopyPlacing)
        {
                ...
                case ObjectMode.Props:
                    {
                        ...
                        for (int i = 0; i < _itemCount; i++)
                        {
                            ...
                            {
                                _position = _position.QuantizeToGameShortGridXYZ();
                            }
                            if (m_placementInfo[i].isValidPlacement)
                            {
                                _propInfo = m_placementInfo[i].propInfo;
                                if (Singleton<PropManager>.instance.CreateProp(out _propID, ref this.m_randomizer, _propInfo, _position, _angle, true))
                                {
                                    PlacementCalculator.SetPropID(i, _propID);

                                    DispatchPlacementEffect(_position, false);
                                }
                            }
                        }
                ...
                case ObjectMode.Trees:
                    {
                        ...
                        for (int i = 0; i < _itemCount; i++)
                        {
                            ...
                            {
                                _treeInfo = m_placementInfo[i].treeInfo;
                                if (Singleton<TreeManager>.instance.CreateTree(out _treeID, ref this.m_randomizer, _treeInfo, _position, true))
                                {
            ...
            for (int i = 0; i < placementCalculator.GetItemCountActual(); i++)
            {
                ...
                else if (PlacementCalculator.GetItemValidPlacement(i, out _itemValid))
                {
                    if (_itemValid == true)
                    {
                        ...
                        {
                            Singleton<TerrainManager>.instance.GetHeightMapping(_position, out _heightMap, out _heightMapping, out _surfaceMapping);
                            PropInstance.RenderInstance(cameraInfo, _propInfo, _id, _position, _scale, _angle, _color, RenderManager.DefaultColorLocation, true, _heightMap, _heightMapping, _surfaceMapping);
                        }
                        else
                        {
                            PropInstance.RenderInstance(cameraInfo, _propInfo, _id, _position, _scale, _angle, _color, RenderManager.DefaultColorLocation, true);
                        }
                    }
                }
            }
            ...
            for (int i = 0; i < placementCalculator.GetItemCountActual(); i++)
            {
                ...
                else if (PlacementCalculator.GetItemValidPlacement(i, out _itemValid))
                {
                    if (_itemValid == true)
                    {
                        ...
                        global::TreeInstance.RenderInstance(null, _treeInfo, _position, _scale, _brightness);
                    }
                }
            }
        ...
        public void DiscoverHoverState(Vector3 position)
        {
            ...
            Vector3 _angleCenter = m_placementInfo[1].position;
            float _angle = hoverAngle;
            Vector3 _anglePos = Circle2.Position3FromAngleXZ(_angleCenter, _angleLocusRadius, _angle);

            Vector3 _spacingPos = fenceMode == true ? m_fenceEndPoints[1] : m_placementInfo[1].position;
        ...
        public void RenderPlacementErrorOverlays(RenderManager.CameraInfo cameraInfo)
        {
            ...
            if (placementCalculator.allItemsValid == true && !_override)
            {
                return;
            }
                ...
                case ActiveState.LockIdle:
                    {
                        ...
                        Color32 _item1Color = hoverState == HoverState.SpacingLocus ? _highlightColor : _baseColor;
                        Vector3 _spacingPos = fenceMode == true ? m_fenceEndPoints[1] : m_placementInfo[1].position;
                        ...
                        {
                            //angle indicator
                            Vector3 _center = m_placementInfo[1].position;
                            float _angle = hoverAngle;
                            Vector3 _anglePos = Circle2.Position3FromAngleXZ(_center, hoverAngleLocusDiameter, _angle);
                            ...
                            _blendColor.a = 88;
                            RenderCircle(cameraInfo, _center, hoverAngleLocusDiameter * 2f, _blendColor, false, true);
                            //angle indicator line
                            Segment3 _angleLine = new Segment3(_center, _anglePos);
                        ...
                        if (true || this.m_cachedControlPoints[1].m_direction != Vector3.zero)
                        {
                            if (PropLineTool.drawMode == PropLineTool.DrawMode.Straight)
                            {
                                if (this.m_cachedControlPoints[1].m_direction != Vector3.zero)
                                {
                                    ...
                                    {
                                        _createPointColor = m_PLTColor_copyPlace;
                                    }
                                    ...
                                    if (placementCalculator.allItemsValid == false)
                                    {
                            ...
                            else if ((PropLineTool.drawMode == PropLineTool.DrawMode.Curved) || (PropLineTool.drawMode == PropLineTool.DrawMode.Freeform)) //not sure if this is necessary
                            {
                ...
                case ActiveState.LockIdle:
                    {
                        if (this.m_keyboardAltDown)
                        {
                            _mainCurveColor = m_PLTColor_copyPlace;
                        }
                        else if (this.m_keyboardCtrlDown)
                        {
                        ...
                        {
                            _mainCurveColor = m_PLTColor_locked;
                        ...
                        else if ((PropLineTool.drawMode == PropLineTool.DrawMode.Curved) || (PropLineTool.drawMode == PropLineTool.DrawMode.Freeform))
                        {

                            RenderElbow(cameraInfo, m_mainArm1, m_mainArm2, 1.00f, 2f, _lockIdleColor, false, false);
                ...
                case ActiveState.ChangeAngle:
                    {
                        if (placementCalculator.angleMode == PlacementCalculator.AngleMode.Dynamic)
                        {
                            ...
                            Vector3 _center = m_placementInfo[1].position;
                            Vector3 _angleVector = m_cachedPosition - _center;
                            _angleVector.y = 0f;
        ...
        private bool DoesPositionNeedUpdating(out Vector3 newPosition)
        {
            ...
            switch (activeState)
            {
                ...
                case ActiveState.LockIdle:
                    {
                        //don't think I need to test LockIdle
                        break; 
                    }
                case ActiveState.MovePointFirst:
                    {
